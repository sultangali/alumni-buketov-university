...
export function KioskSearch({ onClose }: { onClose: () => void }) {
  ...
  const field: CSSProperties = { flex: 1, border: 'none', outline: 'none', background: 'transparent', fontSize: 'var(--t-md)', color: 'var(--c-ink)', fontFamily: 'Manrope, sans-serif' }
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 var(--pad)' }}>
        {results.map((p) => (
          <button key={p.id} onClick={() => { kb.blur(); onClose(); go({ name: 'alumni', id: p.id }) }}
            ...
            <div style={{ width: 52, height: 52, borderRadius: 'var(--r)', background: cardGrad(p.accent), color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Lora, serif', fontWeight: 700 }}>{initials(p.name)}</div>
            <div>
              <div style={{ fontFamily: 'Lora, serif', fontWeight: 600, fontSize: 'var(--t-md)', color: 'var(--c-ink)' }}>{L(p.name)}</div>
