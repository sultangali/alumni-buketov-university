...
router.post('/', asyncHandler(async (req, res) => {
  ...
  const fields = Object.fromEntries(EDITABLE.filter(f => f in body).map(f => [f, body[f]]));
  fields.fac = (fields.fac as string).trim();
  const doc = await Submission.create({ ...fields, status: 'review', submittedAt: new Date().toISOString() });
