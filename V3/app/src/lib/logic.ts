...
export const initials = (o: Loc): string => {
  ...
  const value = ((n[0] || '')[0] || '') + ((n[1] || '')[0] || '')
  // Separate this combination in either alphabet, including mixed-script names.
  return /^[АA][МM]$/i.test(value) ? `${value[0].toUpperCase()}. ${value[1].toUpperCase()}.` : value
}
...
/** Name and faculty constraints compose; an empty constraint is unrestricted. */
export function searchPeople(people: Person[], query: string, faculty: string): Person[] {
  return people.filter(p => (!faculty || p.fac === faculty) && matchesText(query, p.name, p.pos, p.spec))
}
