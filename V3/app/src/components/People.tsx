...
export function Portrait({p}: {p:Person}) {
 ...
 useEffect(()=>{if(!failed || !isNativeKiosk())return;const timer=setTimeout(()=>setFailed(false),15000);return()=>clearTimeout(timer)},[failed,p.photoUrl])
 return <div className="portrait">{p.photoUrl&&!failed?<img src={mediaSrc(p.photoUrl)} alt={L(p.name)} onError={()=>setFailed(true)}/>:<span className={initials(p.name).length>2?'separated-initials':undefined} aria-label={L(p.photoUrl?{ru:'Фотография сейчас недоступна',kz:'Фотосурет қазір қолжетімсіз',en:'Photo currently unavailable'}:{ru:'Фотография не добавлена',kz:'Фотосурет қосылмаған',en:'Photo not added'})}>{initials(p.name)}</span>}</div>
}
